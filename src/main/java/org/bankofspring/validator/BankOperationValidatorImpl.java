...
/*
 * validates a bank operation checks if user is not null (throws RuntimeException if null) checks if
 * amount is positive (throws RuntimeException if null) for debit, check that originating amount has
 * at least the debit amount for credit, checks that not going over maximum amount
 */
@Component
public class BankOperationValidatorImpl implements BankOperationValidator {
	...
	public boolean validateOperation( User user, Account fromAccount,
		...
		if ( type == BankOperationType.DEPOSIT ) {
			return validateCredit( toAccount, amount );
		}
		else if ( type == BankOperationType.WITHDRAWAL ) {
			return validateDebit( fromAccount, amount );
		}
		else if ( type == BankOperationType.TRANSFER ) {
			return validateCredit( toAccount, amount ) && validateDebit( fromAccount, amount );
		}
