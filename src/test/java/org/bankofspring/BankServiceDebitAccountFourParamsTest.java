 ...
 * <code>boolean debit(User loggedInUser, Account fromAccount, Account toAccount, long amount)</code>
 * 
 * @author ram
 *
 */
public class BankServiceDebitAccountFourParamsTest extends
	...
	public void testDebitAccount() {
		...
		assertEquals(
				...
				true, bankService.debit(janice, account10000001,
						account12345678, 400L));
	...
	public void testDebitAccountWithNoUser() {
		try {
			bankService.debit(null, account10000001, account12345678, 1000L);
	...
	public void testDebitAccountWithZeroAmount() {
		try {
			bankService.debit(janice, account10000001, account12345678, 0L);
		} catch (RuntimeException re) {
			assertEquals("RuntimeException message not as expected",
	...
	public void testDebitAccountWithNoAccount() {
		try {
			bankService.debit(janice, null, account12345678, 1000L);
		} catch (RuntimeException re) {
			assertEquals("RuntimeException message not as expected",
	...
	public void testDebitAccountWithLargeAmount() {
		...
		assertEquals(
				...
				false, bankService.debit(janice, account10000001,
						account12345678, 1001L));
		assertEquals(
